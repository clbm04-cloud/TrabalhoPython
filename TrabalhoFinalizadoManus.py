...
import re
import unicodedata
import os # Importar os para verificar se o arquivo existe
...
@st.cache_data
def carregar_dados():
    # O nome do arquivo Excel deve ser LISTA DE MEDICAMENTOS11.xlsx
    FILE_PATH = "LISTA DE MEDICAMENTOS11.xlsx"
    
    # Verifica se o arquivo existe no ambiente de implantação
    if not os.path.exists(FILE_PATH):
        st.error(f"❌ Arquivo de dados '{FILE_PATH}' não encontrado. Por favor, verifique se o arquivo foi incluído no repositório.")
        return pd.DataFrame(), {}
    ...
    try:
        ...
        if linha_cabecalho is None:
            st.error("❌ Nenhuma linha com 'Remédio' encontrada no cabeçalho.")
            return pd.DataFrame(), {}
